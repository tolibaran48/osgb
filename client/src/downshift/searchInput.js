  ...
  return (
    <div>
      <Downshift inputValue={inputValue}
        onChange={(selection) => { onChange({ id: selection.id, value: selection.value }); setInputValue('') }}
          ...
          <div style={{ position: 'relative' }}>
            <button
              ...
              <div className='w-100' style={{ display: 'flex' }}>
                <div className='w-100' style={{ overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', textAlign: alignment }}>{value ? items.find((item) => item.id === value).value : defValue}</div>
                {
                  isOpen ?
            ...
            {isOpen &&
              <div className='downs'>
                <div className='pl-5 pr-5 m-0' style={{ display: 'flex' }}><input type='search' className='form-control form-control-sm' placeholder='Ara...' {...getInputProps({ onChange: (e) => setInputValue(e.target.value) })} style={{ width: '100%', margin: '0.3rem', background: '#eaf4fd4f' }} autoFocus />{reset ? <button style={{ margin: '0.3rem', width: '10px', display: 'flex', justifyContent: 'center' }} type='button' className='form-control form-control-sm' onClick={reset}>X</button> : null}</div>
                ...
                <ul className='down list-group scroll-primary'  {...getMenuProps()} style={{ paddingLeft: '0.5rem', paddingRight: '0.5rem', paddingBottom: '0.3rem', display: 'block' }} >
                  {items
                    ...
                    .map((item, index) => (
                      <li key={item.id} className='form-contol form-control-sm rounded-0 '
                        style={
